...
function QuizQuestions() {
  ...
  useEffect(() => {
    const timerChecker = async () => {
      if (timer <= 0) {
        // Use a normal JavaScript alert instead of Swal
        alert("Time's Up! Your time is over. Submitting the quiz...");
        handleSubmitQuiz();
      }
    }
  ...
  const submitQuiz = async () => {
    // Show confirmation using window.confirm() - returns true or false
    const isConfirmed = window.confirm("Do you really want to submit the quiz?");
    ...
    if (isConfirmed) {
      ...
      try {
        // Submit the quiz data to the database
        const response = await SubmitQuizToDB(user_id, quizId, finalAnswers);
  
        console.log(response);
  
        // Handle response and display alert
        if (!response.success || response.status === 400) {
          alert("Something went wrong! Please contact the development team.");
        } else {
          // Save the exam submission flag in localStorage and navigate to the completion page
  ...
  useEffect(() => {
    cheatingPrevention(async (reason) => {
      ...
      // Show a simple JavaScript alert instead of SweetAlert
      alert(`${reason} Submitting the quiz...`);
  
      // Simulating the 5-second timer from SweetAlert
      setTimeout(async () => {
        await handleSubmitQuiz();
        localStorage.setItem("isExamSubmitted", "yes");
  
        navigate("/completion");
      }, 5000); // 5-second delay to simulate timer behavior
    });
